import { Game, GameItem, GameDetails, Image, Review } from "../interface/metacrticApi.interface";

// function to call the latest games
export async function getBestGames(): Promise<Game[]> {
    const LATEST_GAMES =
  ...
  // function to call a game by slug
  export async function getGameDetails(slug: string): Promise<GameDetails> {
    ...
    // get the card image
    const cardImage = images.find((image: Image) => image.typeName === "cardImage");
